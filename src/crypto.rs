...
// ----------------------------------- MD5 ---------------------------------------
// Frappe stores `tabDocType.migration_hash` = md5(doctype.json bytes) and skips re-importing a
// doctype whose file hash is unchanged. To be hash-compatible (and skip on the first ferro
// migrate), ferro must compute the same md5. MD5 is broken for security but this is a change-
// detection token, not a security primitive — exactly Frappe's use.
